...
> Phase A/B の時点では本作業環境にPDFのテキスト抽出手段がなく、以下は所在確認のみでした。
> **Phase D（2026-09-21）に `common/tools/pdftext.py` で再取得を試み、2件は本文を読めました。**
> 各エントリに結果を追記しています。
...
- source_title   : 令和3年経済センサス‐活動調査 産業別集計（サービス関連産業B）結果の概要
  ...
  accessed_date  : 2026-09-17
  used_for       : **本文の根拠には使用していない。**
                   → **Phase D（2026-09-21）再取得：`pdftext.py` で全文を抽出できた。**
                   ただし中身は「**産業横断的集計 建設・サービス収入の内訳（一次集計）結果の概要**」であり、
                   **宿泊業，飲食サービス業の売上高は収載されていない**ことを確認した。
                   したがって「飲食店の売上高の参照先」としては使えない。本資料の記述は変更不要。
```
...
- source_title   : 令和4年・5年外食産業規模推計値について
  ...
  published_date : 不明（ページ上で確認できず）
  accessed_date  : 2026-09-17
  used_for       : 数値は同じ財団が公表するExcel（§A）から取得している。
                   → **Phase D（2026-09-21）再取得：`pdftext.py` で全文を抽出し、内容を読んだ。**
                   `01_overview.md` 1-2 の令和5年（2023年）の数値が本PDFの記載と
                   **すべて一致すること**を確認した（外食産業市場規模24兆1,512億円／
                   広義の外食産業市場規模31兆7,828億円／給食主体部門20兆2,793億円／
                   料飲主体部門3兆8,719億円／集団給食3兆1,741億円／喫茶店1兆1,892億円／
                   居酒屋・ビヤホール等9,152億円／料亭・バー等1兆7,675億円）。
                   **記述の変更は不要だった。**
```
