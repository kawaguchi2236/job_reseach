...
- source_title   : 診療報酬の算定方法（平成二十年厚生労働省告示第五十九号）本文
  ...
  status         : **内容未読**
  影響する記述    : 01「診療報酬は点数で表され、1点＝10円として金額に換算される」
  ...
  扱い           : 「1点＝10円」は本資料で【Fact】として記述しているが、**上記告示の本文そのものは本作業では原文を確認できていない**。
                   一方、費用の額が「厚生労働大臣が定めるところにより、算定する」ものであること（＝公定価格であること）は
                   健康保険法第76条第2項の条文で確認済みである。後続のQA工程で原文の確認を行うこと。
```
...
- source_title   : 医療経済実態調査（医療機関等調査）
  ...
  url            : （特定できず）
  accessed_date  : 2026-09-17（所在を探したが到達できず）
  status         : **未取得・内容未読**
  影響する記述    : 03 説明用モデル（無床診療所の職員数・1日患者数）
  経緯           : 想定される複数のURLがいずれも404。WebSearchはセッション上限（200回）に達しており使用できなかった。
  扱い           : このため `03_company-structure.md` のモデルA・モデルBの職員数・患者数は
                   **統計に基づく値ではなく、説明用の架空設定（【Example】）** として明記している。
                   モデルの「規模感」の妥当性を統計で裏づける作業は、後続のQA工程で本調査を取得して行うこと。
```
