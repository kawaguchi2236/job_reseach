...
- **労働保険の年度更新**：`13_periodic-work.md` で触れているが**根拠条文を確認していない**ため、
  条文引用はせず「年度ごとの手続き。具体的な期限・様式は所管官庁の案内で確認する `【Varies】`」
  という書き方に留めている。**出典が取れれば追記すること。**
  （参考：他業界では **労働保険の保険料の徴収等に関する法律 `344AC0000000084` 第19条第1項**
  ＝「次の保険年度の**六月一日から四十日以内**」を確認済み。QAで整合を取ること）
