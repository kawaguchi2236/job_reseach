# 19_sources.md — 小売業 出典一覧（初版：Phase A）

> 本ファイルは `common/methodology.md` §10 の形式に従って、
> `01_overview.md` / `02_industry-structure.md` / `03_company-structure.md` / `04_end-to-end-process.md`
> で使用した出典を記録したものです。
>
> **記載しているURLは、2026-09-16 に実際にアクセスし、内容を確認したもののみです。**
> 後続フェーズ（Phase B / C / D）で出典を追加する場合は、本ファイルに追記してください。

---

## 1. 政府統計

```
- source_title   : 令和3年経済センサス‐活動調査 産業別集計（卸売業，小売業に関する集計）結果の概要
  organization   : 総務省・経済産業省
  url            : https://www.stat.go.jp/data/e-census/2021/kekka/pdf/oroshikouri_outline.pdf
  published_date : 2023年3月28日（令和5年3月28日）公表。
                   調査基準日＝2021年6月1日（事業所数・従業者数）、
                   年間商品販売額＝2020年（令和2年）1年間
  accessed_date  : 2026-09-16
  used_for       : ・小売業の事業所数（880,031事業所）、従業者数（7,540,345人）
                     ［01_overview 1-1／1-2］
                   ・小売業の年間商品販売額（133兆2,575億円）、卸売業の年間商品販売額
                     （389兆3,883億円）、卸売業の事業所数（348,889事業所）［01_overview 1-1］
                   ・小売業の産業小分類別 事業所数・従業者数・年間商品販売額
                     （各種食料品小売業22兆9,651億円、自動車小売業16兆5,869億円、
                      医薬品・化粧品小売業13兆293億円 等）［01_overview 1-1／1-6］
                   ・商品販売形態別年間商品販売額（店頭販売84.7％、インターネット販売4.5％、
                     訪問販売3.7％、通信・カタログ販売3.1％、自動販売機0.9％）［01_overview 1-6］
                   ・無店舗小売業の事業所数（通信販売・訪問販売小売業31,353事業所 等）［01_overview 1-6］
                   ・小売業の売場面積（1億3,695万㎡）［01_overview 1-1 関連］
                   ・卸売業と小売業の格付方法（卸売販売額と小売販売額の多いほうで決定）
                     ［01_overview 1-1］
  note           : 「事業所数、従業者数」は格付不能事業所を含むため内訳の積み上げと一致しない。
                   従業者数に臨時雇用者は含まない。年間商品販売額は原則消費税込み。
                   本調査は国税庁法人番号公表サイト情報を用いて調査名簿を拡充しており、
                   過去調査との単純な時系列比較はできない旨が原典に明記されている。
```

```
- source_title   : 商業動態統計速報（2025年11月分）統計表（PDF版・Excel版）
  organization   : 経済産業省（大臣官房調査統計グループ サービス動態統計室）
  url            : https://www.e-stat.go.jp/stat-search/files?page=1&layout=dataset&query=prunes&stat_infid=000040392381
                   （PDF）https://www.e-stat.go.jp/stat-search/file-download?statInfId=000040392381&fileKind=2
                   （Excel）https://www.e-stat.go.jp/stat-search/file-download?statInfId=000040392381&fileKind=0
  published_date : 2025年12月26日
  accessed_date  : 2026-09-16
  used_for       : ・2024年（暦年）の小売業販売額 167兆1,530億円（前年比＋2.5％）、
                     卸売業販売額 445兆9,380億円、商業計 613兆900億円［01_overview 1-1］
                   ・2024年（暦年）の業態別販売額・店舗数
                     百貨店 6兆3,282億円／183事業所、スーパー 16兆529億円／5,976事業所、
                     百貨店＋スーパー計 22兆3,811億円／6,159事業所、
                     コンビニエンスストア 12兆8,887億円／55,988店、
                     ドラッグストア 8兆9,199億円／19,664店、
                     家電大型専門店 4兆7,288億円／2,670店、
                     ホームセンター 3兆3,988億円／4,531店［01_overview 1-6］
                   ・ドラッグストアの2024年 商品別販売額（食品2兆9,397億円が最大 ほか）
                     ［01_overview 1-6］
                   ・百貨店・スーパーの売場面積1㎡当たり販売額 79.3万円、
                     従業者一人当たり販売額 3,678.9万円（いずれも2024年、百貨店＋スーパー計）
                     ［03_company-structure 3-1の検算］
  note           : 経済センサスとは調査対象・集計方法が異なるため、両者の金額は一致しない。
                   2025年1月調査から2021年経済センサス結果に基づく基準改定が行われており、
                   時系列にはリンク係数による接続が必要である旨が原典に記載されている。
```

---

## 2. 業界団体の調査

```
- source_title   : 2024年 スーパーマーケット年次統計調査 報告書
  organization   : 一般社団法人全国スーパーマーケット協会／一般社団法人日本スーパーマーケット協会／
                   オール日本スーパーマーケット協会
  url            : https://www.super.or.jp/wp/wp-content/uploads/2024/10/2024nenji-tokei.pdf
  published_date : 2024年10月
  accessed_date  : 2026-09-16
  used_for       : ・商品カテゴリー別 目標とする利益率（惣菜38.5％、畜産28.4％、水産28.2％、
                     青果23.2％、日配品23.0％、非食品21.5％、一般食品19.9％）
                     ［01_overview 1-5／03_company-structure 3-1］
                   ・商品カテゴリー別 ロス率（惣菜10.6％、水産8.6％、畜産6.6％、日配品4.0％、
                     青果3.2％、一般食品1.7％、非食品1.5％）［01_overview 1-5／03／04］
                   ・商品カテゴリー別 廃棄率（金額ベース：惣菜2.4％、水産2.2％、青果1.3％、
                     畜産1.2％、日配品1.0％、一般食品0.6％、非食品0.4％）［01_overview 1-5／04］
                   ・商品カテゴリー別 売上高構成比（一般食品24.3％、日配品19.0％、青果15.8％、
                     畜産13.8％、水産10.9％、惣菜10.9％、非食品5.4％）［03_company-structure 3-1］
                   ・1店舗あたり正社員数 14.0人、管理職者数 2.9人［01_overview 1-7／03］
                   ・パート・アルバイト比率（店舗勤務71.2％、本部・その他勤務25.5％）
                     ［01_overview 1-7／03］
                   ・平均客単価（平日2,094.2円／土日祝2,437.8円）、
                     1人あたり平均買上点数（平日9.5点／土日祝10.8点）［01_overview 1-4／03］
                   ・バックヤード比率 平均30.5％［03_company-structure 3-1／3-5］
                   ・専用駐車場台数 平均130.9台、売場1,000㎡あたり93.5台［03］
                   ・平均営業時間 10.7時間、24時間営業店 2.4％［03］
                   ・売場1,000㎡あたりレジ台数 平均6.4台［03］
                   ・売場1,000㎡あたり正社員数 平均12.8人［03 関連］
                   ・現金以外の決済手段の導入率 95.6％［04_end-to-end-process A-8］
                   ・万引き防止策実施率 92.1％［01_overview 1-5］
                   ・パート・アルバイトの上限年齢 平均71.3歳［03_company-structure 3-4］
  note           : 調査対象は国内にスーパーマーケットを保有する企業932社、
                   有効回答294社（回収率31.5％）、調査実施期間2024年7月〜8月。
                   本文中で「業界推計値」と記載された数値を優先して引用している。
```

```
- source_title   : 2024年 スーパーマーケット年次統計調査 報告書
                   Ⅲ．スーパーマーケット経営指標レポート
  organization   : 一般社団法人全国スーパーマーケット協会 ほか2団体
                   （集計データ提供：株式会社帝国データバンク 企業財務データベース「COSMOS1」）
  url            : https://www.super.or.jp/wp/wp-content/uploads/2024/10/2024nenji-tokei.pdf
  published_date : 2024年10月
  accessed_date  : 2026-09-16
  used_for       : ・売上高総利益率（粗利率）全体平均 26.34％、売上高30億円未満 25.99％
                     ［01_overview 1-5／03_company-structure 3-1］
                   ・売上高営業利益率 全体平均 1.39％、売上高30億円未満 ▲0.32％、
                     1,000億円以上 3.17％［01_overview 1-5／1-7／03］
                   ・売上高経常利益率 全体平均 1.98％、売上高30億円未満 0.57％［03 関連］
                   ・労働分配率（人件費÷売上総利益）全体平均 46.74％、
                     売上高30億円未満 50.81％、1,000億円以上 35.86％
                     ［01_overview 1-7／03_company-structure 3-1／3-4］
  note           : 集計対象は「食品スーパーマーケット年鑑」収録企業のうち
                   2023年4月〜2024年3月に決算を迎えた335社の最新決算データ。
                   各指標の有効回答企業数は指標により異なる（総利益率327社、営業利益率331社、
                   労働分配率284社）。
```

```
- source_title   : キャッシュレス・ロードマップ2024
  organization   : 一般社団法人キャッシュレス推進協議会
  url            : https://paymentsjapan.or.jp/wp-content/uploads/2024/12/roadmap2024.pdf
  published_date : 2024年12月
  accessed_date  : 2026-09-16
  used_for       : ・日本のキャッシュレス決済比率 39.3％（2023年、金額ベース、対前年比＋3.3ポイント）
                   ・2023年のキャッシュレス支払額 126.7兆円
                     ［04_end-to-end-process A-8］
  note           : 比率の算出に用いられている原データは、民間最終消費支出（内閣府「国民経済計算」）、
                   クレジットカード（日本クレジット協会）、デビットカード・電子マネー（日本銀行「決済動向」）、
                   コード決済（キャッシュレス推進協議会「コード決済利用動向調査」）である旨が
                   原典に明記されている。
```

---

## 3. 法令

```
- source_title   : 中小企業基本法（昭和38年法律第154号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/api/1/lawdata/338AC0000000154
  published_date : 制定1963年（昭和38年）。参照したのは現行条文
  accessed_date  : 2026-09-16
  used_for       : ・第2条第1項第4号：小売業の中小企業者＝資本金5,000万円以下 または 従業員50人以下
                   ・第2条第1項第2号：卸売業＝資本金1億円以下 または 従業員100人以下（比較のため）
                   ・第2条第1項第1号：製造業その他＝資本金3億円以下 または 従業員300人以下（比較のため）
                   ・第2条第5項：小規模企業者＝商業・サービス業では従業員5人以下
                     ［01_overview 1-7／03_company-structure 3-1］
```

```
- source_title   : 卸売市場法（昭和46年法律第35号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/api/1/lawdata/346AC0000000035
  published_date : 制定1971年（昭和46年）。参照したのは現行条文
  accessed_date  : 2026-09-16
  used_for       : ・第1条（目的）、第2条（定義）：卸売市場、生鮮食料品等、開設者、
                     卸売業者、仲卸業者の定義［02_industry-structure 11-2］
```

```
- source_title   : 食品衛生法（昭和22年法律第233号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/api/1/lawdata/322AC0000000233
  published_date : 制定1947年（昭和22年）。参照したのは現行条文
  accessed_date  : 2026-09-16
  used_for       : ・第54条〜第55条：公衆衛生に影響が著しい営業についての都道府県知事の許可
                   ・第57条：許可対象外の営業についての都道府県知事への届出
                     ［02_industry-structure 11-5］
  note           : 具体的な許可業種・施設基準は政省令および都道府県条例で定められる。
                   詳細は 15_rules-customs.md で追加調査すること。
```

```
- source_title   : 大規模小売店舗立地法（平成10年法律第91号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/api/1/lawdata/410AC0000000091
  published_date : 制定1998年（平成10年）。参照したのは現行条文
  accessed_date  : 2026-09-16
  used_for       : ・第1条（目的）、第2条（店舗面積・大規模小売店舗の定義）
                     ［02_industry-structure 11-5］
  note           : 基準面積は第3条により政令で定めるとされており、法律本文には数値の記載がない。
                   具体的な基準面積は 15_rules-customs.md で政令を確認して記載すること。
```

---

## 4. Phase A で確認できなかった事項（後続フェーズへの申し送り）

以下は本文中で `【Varies】` として質的に記述するにとどめ、**数値の断定を避けた**論点です。
Phase B / C / D で一次情報が取得できた場合は、本ファイルに出典を追記のうえ本文を更新してください。

| # | 論点 | 現状の扱い | 調査すべき情報源の候補 |
|---|---|---|---|
| 1 | 小売業の企業数・中小企業数（事業所数ではなく企業単位） | 記載せず | 中小企業庁「中小企業・小規模事業者の数」産業別規模別企業数。**2026-09-16時点で chusho.meti.go.jp は取得できず** |
| 2 | BtoC-EC市場規模・EC化率（物販系） | `01_overview 1-6` では経済センサスの「インターネット販売」の値のみを使用 | 経済産業省「電子商取引に関する市場調査」。**2026-09-16時点で meti.go.jp の直接取得ができず** |
| 3 | キャッシュレス決済の**加盟店手数料率** | `【Varies】` として率を記載せず | キャッシュレス推進協議会、経済産業省のキャッシュレス関連検討会資料 |
| 4 | キャッシュレス決済の**入金サイクル**（締めから入金までの日数） | `【Varies】` として日数を記載せず | 各決済事業者の加盟店規約、業界団体資料 |
| 5 | 業態別の**粗利率**（ドラッグストア・専門店・EC 等） | 食品スーパー以外は `【Typical】`／`【Inference】` の質的記述にとどめた | 各業界団体の統計、上場企業の有価証券報告書 |
| 6 | 小売業の**非正規雇用比率**（食品スーパー以外を含む業界全体） | 食品スーパーの値（71.2％）のみ引用 | 総務省「労働力調査」産業別・雇用形態別 |
| 7 | 1店舗あたりの**取扱アイテム数（SKU数）** | `03_company-structure` でモデル設定値（約1万5,000品目）として明記。**統計値ではない** | 業界団体調査、各社IR資料 |
| 8 | 商品分野別の**リードタイム・発注締め時刻** | `【Varies】` として具体値を記載せず | 卸売業界団体の資料、個別企業の取引条件（一般化は困難） |
| 9 | 書籍・雑誌の**返品制度／再販売価格維持制度**の詳細 | `02_industry-structure 11-2` で `【Varies】` として概略のみ | 公正取引委員会、出版業界団体の資料 |
| 10 | 万引き（窃盗）の認知件数・小売業の被害実態 | 記載せず（`【Fact】`として万引き防止策実施率92.1％のみ引用） | 警察庁「犯罪情勢」統計 |

---

## 5. 出典の短縮表記（本文中で使用しているもの）

| 本文中の表記 | 対応する出典 |
|---|---|
| （出典：令和3年経済センサス‐活動調査／総務省・経済産業省） | 上記 1.の1件目 |
| （出典：商業動態統計速報 2025年11月分／経済産業省） | 上記 1.の2件目 |
| （出典：2024年スーパーマーケット年次統計調査報告書／全国スーパーマーケット協会ほか） | 上記 2.の1件目・2件目 |
| （出典：2024年スーパーマーケット年次統計調査報告書 経営指標レポート） | 上記 2.の2件目 |
| （出典：キャッシュレス・ロードマップ2024／一般社団法人キャッシュレス推進協議会） | 上記 2.の3件目 |
| （出典：中小企業基本法／e-Gov法令検索） | 上記 3.の1件目 |
| （出典：卸売市場法／e-Gov法令検索） | 上記 3.の2件目 |
| （出典：食品衛生法／e-Gov法令検索） | 上記 3.の3件目 |
| （出典：大規模小売店舗立地法／e-Gov法令検索） | 上記 3.の4件目 |

---

## 6. 追加出典（`10_documents.md`〜`16_glossary.md` で使用）

> 以下はすべて **2026-09-16 に実際にアクセスして内容を確認**したものです。
> e-Gov法令検索については、法令APIで条文本体を取得して確認しています。

### 6-1. 法令（e-Gov法令検索）

```
- source_title   : 計量法（平成4年法律第51号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/404AC0000000051
  published_date : 1992年5月20日公布（現行：2025年6月1日施行時点）
  accessed_date  : 2026-09-16
  used_for       : ・第12条第1項（特定商品の計量／量目公差）［11_systems 11-6、15_rules-customs 15-3］
                   ・第16条第1項（検定証印等が付された特定計量器の使用）［15_rules-customs 15-3］
                   ・第19条第1項（都道府県知事等が行う定期検査）
                     ［11_systems 11-6、13_periodic-work 13-9、15_rules-customs 15-3］
```

```
- source_title   : 計量法施行令（平成5年政令第329号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/405CO0000000329
  published_date : 1993年10月6日公布（現行：2025年9月5日施行時点）
  accessed_date  : 2026-09-16
  used_for       : ・第10条（定期検査の対象＝非自動はかり、分銅及びおもり、皮革面積計）
                   ・第11条（定期検査の実施時期の政令で定める期間＝非自動はかり等は「二年」）
                     ［10_documents 10-2(7)、11_systems 11-6、13_periodic-work 13-9、15_rules-customs 15-3］
```

```
- source_title   : 特定商品の販売に係る計量に関する政令（平成5年政令第249号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/405CO0000000249
  published_date : 1993年7月9日公布（現行：2017年6月21日施行時点）
  accessed_date  : 2026-09-16
  used_for       : ・第1条（特定商品）、第2条（特定物象量）、第3条（量目公差）
                     ［11_systems 11-6、15_rules-customs 15-3］
```

```
- source_title   : 消防法施行規則（昭和36年自治省令第6号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/336M50000008006
  published_date : 1961年公布（現行）
  accessed_date  : 2026-09-16
  used_for       : ・第3条第10項（令別表第一（四）項＝物品販売店舗を含む防火対象物の防火管理者は、
                     消火訓練及び避難訓練を年2回以上実施しなければならない）
                   ・第3条第11項（訓練実施にあたりあらかじめ消防機関に通報）
                     ［13_periodic-work 13-9、15_rules-customs 15-11(3)］
```

```
- source_title   : 労働保険の保険料の徴収等に関する法律（昭和44年法律第84号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/344AC0000000084
  published_date : 1969年公布（現行）
  accessed_date  : 2026-09-16
  used_for       : ・第15条第1項（概算保険料の納付＝保険年度の6月1日から40日以内に申告・納付）
                     ［13_periodic-work 13-9］
```

```
- source_title   : 労働安全衛生規則（昭和47年労働省令第32号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/347M50002000032
  published_date : 1972年公布（現行）
  accessed_date  : 2026-09-16
  used_for       : ・第44条（定期健康診断＝常時使用する労働者に対し1年以内ごとに1回）
                     ［13_periodic-work 13-9、15_rules-customs 15-9］
```

```
- source_title   : 労働基準法（昭和22年法律第49号）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/322AC0000000049
  published_date : 1947年公布（現行）
  accessed_date  : 2026-09-16
  used_for       : ・第39条第7項（年10日以上の年次有給休暇が付与される労働者について、
                     基準日から1年以内に5日、使用者が時季を定めて与えなければならない）
                     ［13_periodic-work 13-9、15_rules-customs 15-9］
```

> **条文本文を直接確認していない法令**（本文中では法令名のみを示し、
> 条番号を挙げた箇所は下記の官公庁解説ページを根拠としています）
> 所得税法（第183条：源泉所得税の翌月10日納付／第190条：年末調整）、
> 健康保険法（第164条：保険料の翌月末日納付）、法人税法（第74条）、消費税法（第45条・第63条）、
> 労働安全衛生法（第12条：衛生管理者）、最低賃金法、労働契約法、
> 食品衛生法、食品表示法、景品表示法、独占禁止法、古物営業法、個人情報保護法、
> 容器包装リサイクル法、特定商取引法、
> 二十歳未満ノ者ノ飲酒ノ禁止ニ関スル法律、二十歳未満ノ者ノ喫煙ノ禁止ニ関スル法律
> （後2者は e-Gov法令検索の収録ページ https://laws.e-gov.go.jp/law/133AC1000000033 を確認）

### 6-2. 官公庁の制度解説

```
- source_title   : HACCP（ハサップ）／食品衛生法の改正について
  organization   : 厚生労働省
  url            : https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/kenkou_iryou/shokuhin/haccp/index.html
                   https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000197196.html
  published_date : 制度：2018年6月13日公布／2021年6月1日完全施行
  accessed_date  : 2026-09-16
  used_for       : ・HACCPに沿った衛生管理の制度化（2021年6月1日完全施行）
                   ・小規模営業者等は手引書を参考に簡略化したアプローチが可能
                   ・営業許可制度の見直しと営業届出制度の創設
                     ［10_documents 10-2(7)、13_periodic-work、15_rules-customs 15-1］
```

```
- source_title   : 食物アレルギー表示に関する情報／くるみの特定原材料への追加等について
  organization   : 消費者庁
  url            : https://www.caa.go.jp/policies/policy/food_labeling/food_sanitation/allergy/
  published_date : くるみの追加：2023年（令和5年）3月／経過措置期間：2025年3月31日まで
  accessed_date  : 2026-09-16
  used_for       : ・特定原材料8品目（えび、かに、くるみ、小麦、そば、卵、乳、落花生）
                     ［15_rules-customs 15-2］
```

```
- source_title   : 【令和６年10月１日施行】改正景品表示法の概要／不当な価格表示についての
                   景品表示法上の考え方／「おとり広告に関する表示」等の運用基準
  organization   : 消費者庁
  url            : https://www.caa.go.jp/policies/policy/representation/fair_labeling/
                   https://www.caa.go.jp/policies/policy/representation/fair_labeling/movie_explanation/assets/representation_cms216_240917_02.pdf
                   https://www.caa.go.jp/policies/policy/representation/fair_labeling/guideline/pdf/100121premiums_35.pdf
                   https://www.caa.go.jp/policies/policy/representation/fair_labeling/guideline/pdf/100121premiums_31.pdf
  published_date : 改正法施行：2024年（令和6年）10月1日
  accessed_date  : 2026-09-16
  used_for       : ・二重価格表示、おとり広告の考え方
                   ・2024年10月1日施行の改正（確約手続の導入、課徴金の加算）
                     ［10_documents 10-2(2)、15_rules-customs 15-4］
```

```
- source_title   : No.6902「総額表示」の義務付け／消費税における「総額表示方式」の概要
  organization   : 国税庁／財務省
  url            : https://www.nta.go.jp/taxes/shiraberu/taxanswer/shohi/6902.htm
                   https://www.mof.go.jp/tax_policy/summary/consumption/sougakuhyoji_gaiyou.htm
  published_date : 特例の失効：2021年3月31日／総額表示の必要：2021年4月1日〜
  accessed_date  : 2026-09-16
  used_for       : ・総額表示義務（消費税法第63条）［15_rules-customs 15-5(1)］
```

```
- source_title   : インボイス制度について／インボイス制度に関するQ&A（適格簡易請求書の記載事項）
  organization   : 国税庁
  url            : https://www.nta.go.jp/taxes/shiraberu/zeimokubetsu/shohi/keigenzeiritsu/invoice_about.htm
                   https://www.nta.go.jp/taxes/shiraberu/zeimokubetsu/shohi/keigenzeiritsu/pdf/qa/58.pdf
  published_date : 制度開始：2023年（令和5年）10月1日
  accessed_date  : 2026-09-16
  used_for       : ・インボイス制度の開始日
                   ・小売業など不特定多数に販売する事業は適格簡易請求書を交付できること、
                     交付先の氏名・名称の記載が不要であること
                     ［11_systems 11-10、15_rules-customs 15-5(2)］
```

```
- source_title   : 酒類の販売管理／酒類の免許
  organization   : 国税庁
  url            : https://www.nta.go.jp/taxes/sake/hambai/mokuji.htm
                   https://www.nta.go.jp/taxes/sake/menkyo/mokuji.htm
  published_date : 酒類販売管理研修の受講義務：2017年6月1日施行
  accessed_date  : 2026-09-16
  used_for       : ・酒類販売業免許（販売場ごと）
                   ・酒類販売管理者の選任（過去3年以内に研修を受けた者から）
                   ・3年を超えない期間ごとの研修受講
                     ［13_periodic-work 13-9、15_rules-customs 15-6］
```

```
- source_title   : 医薬品の販売制度
  organization   : 厚生労働省
  url            : https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000082514.html
  published_date : 不明（制度解説ページ）
  accessed_date  : 2026-09-16
  used_for       : ・一般用医薬品の区分（第1類・第2類・第3類）
                   ・第2類・第3類は薬剤師のほか登録販売者も販売等が可能
                   ・店舗管理者の要件
                     ［11_systems 11-16、15_rules-customs 15-7］
```

```
- source_title   : 大規模小売業者による納入業者との取引における特定の不公正な取引方法
                   （平成17年5月13日 公正取引委員会告示第11号）／
                   優越的地位の濫用に関する独占禁止法上の考え方／
                   大規模小売業者との取引に関する納入業者に対する実態調査報告書
  organization   : 公正取引委員会
  url            : https://www.jftc.go.jp/dk/guideline/tokuteinounyu.html
                   https://www.jftc.go.jp/hourei_files/yuuetsutekichii.pdf
                   https://www.jftc.go.jp/soshiki/kyotsukoukai/kenkyukai/dk-kondan/kaisai_h29_files/209-2-2.pdf
  published_date : 告示：2005年（平成17年）5月13日／
                   優越的地位の濫用の考え方：2010年11月30日（2017年6月16日改正）
  accessed_date  : 2026-09-16
  used_for       : ・「大規模小売業者」の定義（売上高100億円以上／東京都特別区・指定都市内3,000㎡以上／
                     その他市町村1,500㎡以上）
                   ・規制される10の行為類型（不当な返品、不当な値引き、不当な委託販売取引、
                     特売商品等の買いたたき、特別注文品の受領拒否、押し付け販売等、
                     納入業者の従業員等の不当使用、不当な経済上の利益の収受、
                     要求拒否の場合の不利益な取扱い、報告に対する不利益な取扱い）
                   ・センターフィーに関する実態調査
                     ［15_rules-customs 15-10、15-14、15-15］
```

```
- source_title   : 大規模小売店舗立地法について
  organization   : 経済産業省
  url            : https://www.meti.go.jp/policy/economy/distribution/daikibokouritenporittiho.html
  published_date : 法施行：2000年（平成12年）6月1日／
                   配慮事項の指針：平成19年2月1日経済産業省告示第16号
  accessed_date  : 2026-09-16
  used_for       : ・大規模小売店舗＝一の建物内の店舗面積が1,000㎡を超えるもの
                   ・配慮事項（交通・騒音・廃棄物など生活環境に関する事項）
                     ［15_rules-customs 15-11(1)］
```

```
- source_title   : プラスチック製買物袋有料化 2020年7月1日スタート
  organization   : 経済産業省
  url            : https://www.meti.go.jp/policy/recycle/plasticbag/plasticbag_top.html
  published_date : 有料化開始：2020年（令和2年）7月1日
  accessed_date  : 2026-09-16
  used_for       : ・容器包装リサイクル法に基づく省令改正によるレジ袋有料化
                   ・対象＝持ち手のついたプラスチック製買物袋
                     ［15_rules-customs 15-8(1)］
```

```
- source_title   : 同一労働同一賃金ガイドライン
  organization   : 厚生労働省
  url            : https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000190591.html
  published_date : 改正省令・改正指針の公布：2026年（令和8年）4月28日／
                   適用開始：2026年（令和8年）10月1日
  accessed_date  : 2026-09-16
  used_for       : ・パートタイム・有期雇用労働法に基づく不合理な待遇差の禁止
                   ・改正同一労働同一賃金ガイドラインが2026年10月1日から適用されること
                     ［15_rules-customs 15-9］
```

```
- source_title   : 防火管理者が必要な防火対象物と資格
  organization   : 東京消防庁
  url            : https://www.tfd.metro.tokyo.lg.jp/lfe/office_adv/jissen/p04.html
  published_date : 不明（制度解説ページ）
  accessed_date  : 2026-09-16
  used_for       : ・物品販売店舗は特定用途の防火対象物にあたり、
                     収容人員30人以上で防火管理者の選任が必要（消防法第8条）
                     ［13_periodic-work 13-9、15_rules-customs 15-11(3)］
```

```
- source_title   : 特定計量器を利用する場合
  organization   : 経済産業省（産業技術環境局 計量行政室）
  url            : https://www.meti.go.jp/policy/economy/hyojun/techno_infra/12_gaiyou_keiryouki1.html
  published_date : 不明（制度解説ページ）
  accessed_date  : 2026-09-16
  used_for       : ・取引・証明に用いる特定計量器には検定証印等が必要であること（計量法第16条第1項）
                     ［15_rules-customs 15-3］
```

### 6-3. 追加分で確認できなかった事項（後続フェーズへの申し送り）

| # | 論点 | 現状の扱い | 調査すべき情報源の候補 |
|---|---|---|---|
| 11 | キャッシュレス決済の**手数料率・入金サイクル**の具体値 | `【Varies】` として数値を示さず（`11_systems 11-7`、`12_business-model 12-8`、`14_kpi 14-6`） | 各決済事業者の加盟店規約、キャッシュレス推進協議会 |
| 12 | **地域別最低賃金**の金額 | `【Varies】` として金額を示さず（`15_rules-customs 15-9`） | 厚生労働省「地域別最低賃金の全国一覧」（毎年改定） |
| 13 | 食品衛生法上の**営業許可の有効期間** | `【Varies】` として「自治体・許可の種類により異なる」とのみ記載 | 各自治体保健所。全国一律の値がないため一般化は困難 |
| 14 | **欠品率・リピート率**の業界平均値 | `【Varies】` として数値を示さず（`14_kpi 14-3(13)`、`14-1(7)`） | 定義が企業ごとに異なり、公的統計が存在しない |
| 15 | **中小小売業における帳票・システムの媒体構成比**（紙／表計算／システム） | `【Varies】` として比率を示さず（`10_documents 10-4`、`11_systems`） | 中小企業実態基本調査等。ただし業態別の分離は困難 |
| 16 | **委託仕入・消化仕入の業態別採用比率** | `【Typical】`／`【Varies】` の質的記述にとどめた（`15_rules-customs 15-13`） | 業界団体調査、上場企業の有価証券報告書（会計方針） |

### 6-4. 追加分の短縮表記

| 本文中の表記 | 対応する出典 |
|---|---|
| （出典：計量法／e-Gov法令検索） | 6-1 |
| （出典：計量法施行令／e-Gov法令検索） | 6-1 |
| （出典：消防法施行規則／e-Gov法令検索） | 6-1 |
| （出典：労働安全衛生規則／e-Gov法令検索） | 6-1 |
| （出典：労働基準法／e-Gov法令検索） | 6-1 |
| （出典：HACCP（ハサップ）／厚生労働省） | 6-2 |
| （出典：食物アレルギー表示に関する情報／消費者庁） | 6-2 |
| （出典：景品表示法／消費者庁） | 6-2 |
| （出典：国税庁「No.6902『総額表示』の義務付け」） | 6-2 |
| （出典：国税庁「インボイス制度に関するQ&A」） | 6-2 |
| （出典：国税庁「酒類の販売管理」） | 6-2 |
| （出典：医薬品の販売制度／厚生労働省） | 6-2 |
| （出典：公正取引委員会） | 6-2 |
| （出典：大規模小売店舗立地法について／経済産業省） | 6-2 |
| （出典：同一労働同一賃金ガイドライン／厚生労働省） | 6-2 |
| （出典：東京消防庁「防火管理者が必要な防火対象物と資格」） | 6-2 |

---

## 7. Phase D（QA）で追加した出典

```
- source_title   : 製造委託等に係る中小受託事業者に対する代金の支払の遅延等の防止に関する法律
                   （昭和31年法律第120号。略称：中小受託取引適正化法／通称：取適法。旧称：下請代金支払遅延等防止法）
  organization   : e-Gov法令検索（デジタル庁）
  url            : https://laws.e-gov.go.jp/law/331AC0000000120
  published_date : 1956年6月1日公布／令和7年法律第41号による改正（2025年5月23日公布）は2026年1月1日施行
  accessed_date  : 2026-09-17
  used_for       : ・第2条第1項（製造委託の定義。「業として行う販売の目的物たる物品の製造を他の事業者に
                     委託すること」＝小売業のPB製造委託が該当しうる根拠）［15_rules-customs 15-10(3)、15-20］
                   ・第2条第3項・第5項・第6項（情報成果物作成委託／特定運送委託／「製造委託等」の範囲）
                     ［15_rules-customs 15-10(3)］
                   ・第2条第8項・第9項（委託事業者・中小受託事業者の資本金基準および従業員基準）
                     ［15_rules-customs 15-10(3)］
                   ・第3条第1項（支払期日は受領日から起算して60日以内）［15_rules-customs 15-10(3)］
                   ・第5条第1項第2号（支払遅延の禁止。手形の交付等を含む）［15_rules-customs 15-10(3)］
  note           : 条文本文を e-Gov 法令API（law_data / MainProvision）で直接取得して確認した。
```

```
- source_title   : 中小受託取引適正化法（取適法）について
  organization   : 公正取引委員会
  url            : https://www.jftc.go.jp/partnership_package/toritekihou.html
  published_date : 不明（2026年1月1日施行に関する解説ページ）
  accessed_date  : 2026-09-17
  used_for       : 15_rules-customs 15-10(3)（題名変更・略称・施行日・改正のポイント）
  note           : HTMLページとして内容を確認した。
```
